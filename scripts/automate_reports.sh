...
# --- CONFIGURATION ---
GITHUB_REPO_PATH="/Users/adminhr/qa-auto-infina-b2c-api"  
GITHUB_PAGES_URL="https://realstake.github.io/qa-auto-infina-b2c-api"
...
for COLLECTION in "${COLLECTIONS[@]}"; do
  ...
  newman run "scripts/${COLLECTION}.postman_collection.json" \
    --delay-request 1000 \
    --reporters htmlextra,cli \
    --reporter-htmlextra-export "reports/${REPORT_NAME}" \
    --verbose > "reports/${CLI_OUTPUT}"
  REPORT_FILES+=("${REPORT_NAME}")
  CLI_LOG_FILES+=("${CLI_OUTPUT}")

  # Extract summary block (from ┌ to └)
  SUMMARY=$(awk '/^┌/{flag=1} flag; /^└/{flag=0}' "reports/${CLI_OUTPUT}")
...
# --- 4. Copy Reports and Log Files to GitHub Repo ---
echo "Copying reports and CLI logs to GitHub repo..."
...
# --- 6. Create Comprehensive Slack Report ---
...
# --- 7. Post Comprehensive Report to Slack ---
